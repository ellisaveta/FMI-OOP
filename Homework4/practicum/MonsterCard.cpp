...
MonsterCard::MonsterCard(const std::string Name, const std::string effect, const unsigned int& rarity, const int& a, const int& p) :
	Card(Name, effect, rarity), attackPoints(a), protectPoints(p) {}

MonsterCard::MonsterCard(const MonsterCard & card) :
	Card(card.getName(), card.getEffect(), card.getRarity()), attackPoints(card.getAttackPoints()), protectPoints(card.getProtectPoints()) {}
...
{
	o << card.getName() << '|' << card.getEffect() << '|' << card.getRarity() << '|'
		<< card.getAttackPoints() << '|' << card.getProtectPoints() << '\n';
	return o;
}
