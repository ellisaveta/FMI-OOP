...
PendulumCard::PendulumCard(const std::string n, const std::string e, const unsigned int& r, const int & a, const int & d, const unsigned int & p,
	const CardType & t) : Card(n, e, r), MonsterCard(n, e, r, a, d), MagicCard(n, e, r, t), pendulumScale(p)
{
	if (p < 1 || p > 13)
	{
...
PendulumCard::PendulumCard(const PendulumCard & card) : Card(card.getName(), card.getEffect(), card.getRarity()),
MonsterCard(card.getName(), card.getEffect(), card.getRarity(), card.getAttackPoints(), card.getProtectPoints()),
MagicCard(card.getName(), card.getEffect(), card.getRarity(), card.getType()), pendulumScale(card.getPendulumScale()) {}
...
{
	o << card.getName() << '|' << card.getEffect() << '|' << card.getRarity() << '|' << card.getAttackPoints()
		<<'|' << card.getProtectPoints() << '|' << card.getPendulumScale() << '|';
	switch (card.getType())
	{
	case CardType::TRAP:
		o << "TRAP";
